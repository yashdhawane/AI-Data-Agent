...
type Tab = "overview" | "members" | "sources" | "schema" | "query";
...
export default function Home() {
  ...
  const [result, setResult] = useState<{ sql: string; columns: string[]; rows: Record<string, unknown>[]; rowCount: number } | null>(null);

  useEffect(() => {
    api("/auth/me")
      .then((data) => { setUser(data.user); return Promise.all([api("/data-sources"), api("/users")]); })
      .then(([sources, users]) => { setDataSources(sources); setSelectedSource(sources[0]?.id ?? ""); setMembers(users); })
      .catch(() => undefined)
  ...
  if (user) return <main className={styles.appShell}>
    <aside className={styles.sidebar}><div className={styles.brand}><span className={styles.brandMark}>◒</span><span>Data Agent</span></div><div className={styles.orgBlock}><span className={styles.orgDot} />{user.name ?? user.email}<small>{user.role} workspace</small></div><nav>{(["overview", "members", "sources", "schema", "query"] as Tab[]).map((item) => ((item === "members" || item === "sources") && user.role !== "ADMIN") ? null : <button key={item} className={tab === item ? styles.navActive : ""} onClick={() => { setTab(item); if (item === "schema") void loadMetadata(); }}><span>{({ overview: "⌂", members: "◎", sources: "▣", schema: "◇", query: "✦" } as Record<Tab, string>)[item]}</span>{item === "overview" ? "Overview" : item === "members" ? "Members" : item === "sources" ? "Data sources" : item === "schema" ? "Schema" : "Ask your data"}</button>)}</nav><button className={styles.signOut} onClick={signOut}>Sign out <span>↗</span></button></aside>
